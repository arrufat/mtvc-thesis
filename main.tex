...
The Lagrange multiplier $\lambda$ of the constraint.
...
% The quantised transformed coefficient are obtained as follows:
% \begin{equation}
...
\begin{equation}
	D
	= \int_{\infty}^{\infty} {(x - \hat x)}^2 \P_X (X)\text{d}x
	= \int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
	x^2\text{P}_X (X) \d x
\end{equation}

For the sake of simplicity, \ac{PDF} of $x$ will be assumed to be
Laplacian.

\begin{align}
	D
	&= a \int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
		x^2 \e ^{-b\vert x \vert} \d x \nonumber \\
	&= 2a \int_0^{\nicefrac{\Delta}{2}}
		x^2 \e ^{-b x} \d x \\
...
\begin{align}
	R 
	&= {\Vert \c \Vert}_0 \nonumber \\
	&= N \P_X\left(\vert X \vert \ge \frac{\Delta}{2}\right) \nonumber \\
	&= 2N\int_{\nicefrac{\Delta}{2}}^{\infty}\P_X(x)\d x \\
	&= 2N\left(\frac{1}{2} - \int_0^{\nicefrac{\Delta}{2}}\P_X(x)\d x \right) \nonumber \\
...
\begin{align}
	R
	&= 2N \left(\frac{1}{2}-{\left[-\frac{1}{2}\e^{\nicefrac{-b\Delta}{2}}\right]}
		_0^{\nicefrac{\Delta}{2}}\right) \nonumber \\
	&= 2N \left(\frac{1}{2}-\left[-\frac{1}{2}
