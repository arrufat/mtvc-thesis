...
as demonstrated in Section~\ref{sub:the_zero_norm}:
\begin{equation}
	\c[n] =
	\begin{cases}
...
\end{equation}
Since the expression represents a scalar value, it can be rewritten as:
\begin{equation}
	\A_{opt} = \arg\min\limits_{\A}
...
\begin{equation}
	D
	= \int_{-\infty}^{\infty} {(x - \hat x)}^2 \P_X (X)\dx
	= \int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
	x^2\P_X (X) \d x
\end{equation}
...
simplicity, instead of a generalised Gaussian \ac{PDF}:
\begin{align}
	D
	&= \int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
