...
Moreover, using an orthogonal transform $\A$ for the rows of a block and
another orthogonal transform $\B$ for the resulting columns does not result in
the same signal in the transform domain as if the operations are carried out
...
Moreover if both transforms are related between each other as $\A^T=\B$,
...
Figure~\ref{fig:comb_dct_dst} shows all possible combinations of using one of
the 16 \acp{DTT} as the horizontal matrix and another as the vertical matrix.
This leads to a total of 256 possible transforms.
