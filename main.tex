...
The \ac{MDDT} was proposed during the \ac{HEVC} standardisation phase, but is
was finally discarded in favour of a new transform, the \ac{DST}.
...
specialisation.
In \ac{HEVC}, this means having one adapted transform in each one of the 35
\acp{IPM}.
...
\begin{figure}[tb]
	...
	\caption{Residual differences for different \acsp{IPM}}
...
In the decoder side (figure~\ref{fig:mddt_dec}), the only information needed
...
lack of fast algorithms for the adapted transforms.
...
The experiment carried out in this section is closely related to the
publication in~\cite{arrufat-14-mddt-rdot}.
...
% TODO
\todo{Choose one of the following learning sets}
...
In order to validate intuitively the results of the iterative learning
algorithm for the \ac{RDOT}, presented in
...
during the learning phase, averaged accross all \acp{IPM}, for separable and
...
\begin{figure}[tb]
	...
	\todo{
	\begin{itemize}
		\item Figure showing the learning behaviour.
		\item Compare sep and nsep \ac{RDOT}.
		\item Add marks for sep and nsep \ac{KLT} values.
		\item Add marks for the \ac{DST} and \ac{DCT} used in HEVC.
		\item For which \ac{IPM} \to{} average?
	\end{itemize}
	}
	\subfloat[$4\times4$]{\includegraphics{./figures/rdot_learning_4_plot.pdf}}
	
	\subfloat[$8\times8$]{\includegraphics{./figures/rdot_learning_8_plot.pdf}}
...
the separable version.
As stated in Chapter~\ref{cha:transform_coding}, non-separable transforms are
able to exploit any linear correlation between a pair of pixels in a block,
allowing for sparser signals in the transform domain.
Furthermore, the figure also contains the \ac{RDOT} metric evaluated on the
\ac{KLT}, both separable and non-separable, and the default \ac{HEVC}
transforms.
...
use of the \ac{DST}-VII over the \ac{DCT}-II:
...
with, corroborating some known facts in the \ac{BD}-rate domain:
...
of \ac{BD}-rate than the \ac{KLT}.
Consequently, the \ac{KLT} will not be considered in the upcoming sections.

The following chapter presents an improvement on \ac{MDDT}, which will allow
transforms to be better adapted to intra prediction residuals and provide new
...
Chapter~\ref{cha:the_mode_dependent_directional_transforms} introduced the
...
\begin{figure}[tp]
	\centering
	\includegraphics{./figures/clustering_plot.pdf}
