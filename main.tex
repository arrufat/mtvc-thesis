...
\end{equation}
The threshold value depends only on the Lagrange multiplier $\lambda$,
...
be updated to provide the link between $\x$ and $\c$ while minimising
...
decomposition $\Y=\U\Lambda^{\nicefrac{1}{2}}\V^T$, where $\U$ and $\V$
are orthonormal and $\Lambda$ is diagonal.
...
\begin{equation}
	...
	\right)
\end{equation}
Since $\Lambda$ is a diagonal matrix whose entries are non-negative by
...
The generalised normal distribution \ac{PDF} can be expressed compactly
as:
\begin{equation}
	\frac{\beta}{2\alpha\Gamma \left(\nicefrac{1}{\beta}\right)}
	{\e^{-\left(\nicefrac{\vert x - \mu \vert}{\alpha}\right)}}^\beta
\end{equation}
It is a well-known fact that transformed coefficients have a centred
Laplace-like \ac{PDF}, which can be obtained with $\beta=1$:
\begin{equation}
	\frac{1}{2\alpha}\e^{\nicefrac{-\vert x \vert}{\alpha}}
\end{equation}
In order to simplify the upcoming calculations, the above equation is
rewritten as:
\begin{equation}
	a\e^{-b\vert x \vert}
\end{equation}
...
\begin{equation}
	...
	x^2\P_X (X) \d x
\end{equation}
Note that the integration intervals can be reduced to where the
...
This first approach will consider a Laplacian \ac{PDF} for the sake of
simplicity, instead of a generalised normal \ac{PDF}:
\begin{align}
	D
	&= \int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
		a x^2 \e ^{-b\vert x \vert} \d x \nonumber \\
	&= 2a \int_0^{\nicefrac{\Delta}{2}}
		x^2 \e ^{-b x} \d x \\
		&= \frac{2a}{b^3}{\left[\e^{-bx}(b^2x^2+2bx+2)\right]}
		_0^{\nicefrac{\Delta}{2}} \nonumber
\end{align}
Resulting in:
\begin{equation}
	D = \frac{2a}{b^3}\left[ 2-\e^{\nicefrac{-b\Delta}{2}}
	\left(b^2 \frac{\Delta^2}{4}+b\Delta+2\right)\right]
	\label{eqn:distortion}
\end{equation}
Deriving equation~\ref{eqn:distortion}:
\begin{equation}
...
\begin{align}
	...
	&= 2\int_{\nicefrac{\Delta}{2}}^{\infty}\P_X(x)\d x \\
	&= 2\left(\frac{1}{2} - \int_0^{\nicefrac{\Delta}{2}}\P_X(x)\d x \right) \nonumber \\
	&= 2\left(\frac{1}{2} - \int_0^{\nicefrac{\Delta}{2}}a\e^{-bx}\d x \right) \nonumber
\end{align}
Note that $\displaystyle\frac{a}{b} = \frac{1}{2}$ since the distribution is
Laplacian.
Hence:
\begin{align}
	R
	&= 2 \left(\frac{1}{2}-{\left[-\frac{1}{2}\e^{\nicefrac{-b\Delta}{2}}\right]}
		_0^{\nicefrac{\Delta}{2}}\right) \nonumber \\
	&= 2 \left(\frac{1}{2}-\left[-\frac{1}{2}
		\e^{\nicefrac{-b\Delta}{2}}+\frac{1}{2}\right]\right) \\
	&= \e^{\nicefrac{-b\Delta}{2}} \nonumber
\end{align}
\begin{equation}
	R = \e ^{\nicefrac{-b\Delta}{2}}
	\label{eqn:zero_norm_int}
\end{equation}
Deriving equation~\ref{eqn:zero_norm_int}:
\begin{equation}
...
With both the distortion and the constraint derived, the optimal
Lagrange multiplier can be found as:
\begin{equation}
...
\begin{equation}
	a\frac{\Delta^2}{4}-\lambda\frac{b}{2}=0
\end{equation}
Finally, reminding that $\displaystyle \frac{a}{b}=\frac{1}{2}$:
\begin{equation}
	\lambda = \frac{\Delta^2}{2}
\end{equation}
This proves how the Lagrange multiplier is directly related to the
quantisation step.
...
transform learnings.
This section justifies its use over other rate models, even those closer
to the real rate, such as the entropy.
