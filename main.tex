...
improvements are still required as new technologies emerge:
...
All these above mentioned formats are made possible thanks to the fact that
every ten years, the compression rate doubles for equivalent quality.
...
outperformed H.264/\acs{MPEG}-4 by 50\% in terms of bit-rate savings for
...
Consequently, the focus has been put on improving the \acs{HEVC} standard with
...
standard for those dates.

\bigskip

Since new video coding standards tend appear every 10 years and being in the
beginning of the post-\acs{HEVC} era, first steps in this thesis strive to
achieve important bit-rate savings over \acs{HEVC} by relaxing complexity
constraints.

This thesis is strongly inscribed inside a standardisation context.
As such, first efforts are put on finding new working techniques, namely using
different transform designs, questioning transform separability and using new
kinds of transforms inside the current video coding standard.
Then, the studies point towards making these new techniques involving
...
Work carried out in this thesis contains several objectives:
\begin{itemize}
...
Consequently, this thesis has been organised into the following Chapters:

\begin{enumerate}[labelindent=3.8em,leftmargin=!,label={\bf Chapter \arabic{enumi}}]
	\item starts with an introduction to the basics of video coding and some
		essential concepts inside modern video coders for this thesis.
		The focus is quickly put on the transform stage and its crucial role
		in the video coding scheme.

	\item contains a thorough study on the transform role inside video coding
		applications.

		As mentioned above, the main motivation of this thesis is to improve
		video coding using use of multiple transforms.
		...
		The \ac{RDOT} presented in this thesis is another design method that
		tries to output a signal which is as sparse as possible while
		minimising the distortion introduced by the quantisation.
	...
	\item compares the \ac{KLT} and \ac{RDOT} design methods introduced in the
		...
		This experiment questions the optimality of the \ac{KLT} is questioned
		for video signals.
		Moreover, the impact of transform separability impact on video coding
		in terms of bit-rate savings and coding complexity is revisited.

	\item takes the \ac{MDDT} system one step further by introducing the
		\ac{MDTC} system.
		The main idea is to provide several transforms in each \ac{IPM}.
		These transforms compete against each other in the same way that block
		sizes and \acp{IPM} do.
		\ac{MDTC} systems are able to report notable bit-rate savings but they
		are highly complex in terms of encoding and decoding times, as well as
		in storage requirements for the used transforms, especially for
		...
		improvements.
	\item explores a way of simplifying non-separable \ac{MDTC} systems, which
		provided the most promising results in the previous Chapter, by using
		incomplete transforms.

		Incomplete transforms are a kind of low complexity transforms that,
		instead of using all the basis vectors for the non-separable
		transforms, only the first one is retained, and they are used as a
		companion of the default \acs{HEVC} transform.
	\item presents different methods for reducing the storage requirements of
		\ac{MDTC} systems.
		Proposed methods are based on the fact that not all \acp{IPM} have the
		same behaviour inside \ac{HEVC} and, therefore, do not have the same
		needs with regards the number of transforms, which is reduced to limit
		the impact in encoding complexity and storage.
	...
	\item contains the final experiment carried out in this thesis replaces
		the \ac{RDOT}-based \ac{MDTC} system with another one making use of
		\acp{DTT}.
		...
		decoding complexity with regards to \acs{HEVC}.

	\item is a summary of the work carried out in this thesis.
		Since many different systems offering different trade-offs between
		...
		This allows having a better overview of systems making use of multiple
		transforms.
\end{enumerate}

In conclusion, the motivation of this thesis is to prove the fact that
multiple transforms can provide significant bit-rate savings compared to the
latest video coding standard, \ac{HEVC}.
Substantial gains over \ac{HEVC} can be achieved while overlooking the system
complexity.
For realistic systems, tangible gains can be obtained at reasonable encoder
complexity.
...
The figure provides a partitioning for a certain level of quantisation using
...
order.
Table~\ref{tab:mddt_summary} contains the \acs{ROM} requirements for the \ac{MDDT}
...
\chapter{Real world \acs{MDTC} system}
...
system presented in Chapter~\ref{cha:mdtc} while trying to have minimum impact
on its bit-rate savings.
...
systems are around 4.1\%, which will be used as a reference when comparing the
simplified systems.
...
\begin{itemize}
	\item BC: classes B and C from the \ac{HEVC} test set
...
For illustrative purposes, figure~\ref{fig:residuals_independence} shows the
results from tables~\ref{tab:residuals_independence_4}
...
sequences used during the learning phase (BC and ToS), since they has notable
differences in performances when using their own learning set.
The rest of test set presents similar performance levels for both learning
sets.
...
equally.
For this reason, \ac{HEVC} favours the signalling of the \acp{IPM} that are
most often used during the encoding of a sequence in terms of signalling.
...
extra signalling, since the encoder will know how many transforms are
available for each \ac{IPM}.
...
\subsection{Non-homogeneous \acs{MDTC} design and performances}
\label{sub:non_homogeneous_mdtc_performances}
...
algorithm~\ref{alg:vmdtc_iter} presents a systematic way of designing an
heterogeneous \ac{MDTC} system, using a gradient approach.
...
The mains systems that are compared are the homogeneous and heterogeneous
...
Iterative heterogeneous \ac{MDTC} systems have more flexibility in terms of
...
The heterogeneous $4\times4$ system is able to achieve the same performances
as the homogeneous system, with less \ac{ROM}.

The interest of the heterogeneous systems is clearer when using $8\times8$
...
the same \ac{BD}-rate in homogeneous and heterogeneous systems.
...
and $8\times8$ systems, in both homogeneous and heterogeneous ways,
represented using green lines.
Again, for these systems, especially on low \ac{ROM} configurations, a factor
...
Consequently, the optimal combinations of heterogeneous \ac{MDTC} systems have
been found by setting a \acs{ROM} specification and selecting the combination
of heterogeneous $4\times4$ and $8\times8$ systems that provide the best
\ac{BD}-rate without exceeding the specified \acs{ROM}.
...
The selected \ac{ROM} requirements for the systems presented in
figure~\ref{fig:vmdtc_iter} are detailed in
table~\ref{tab:heterogeneous_mdtc}.
Information about their actual \ac{ROM} requirements, \ac{BD}-rate and
...
\begin{table}[tb]
	...
	\end{tabular}
	\caption{Heterogeneous \acs{MDTC} systems with \acs{ROM} constraints}
	\label{tab:heterogeneous_mdtc}
\end{table}

Nonetheless, learning the $4\times4$ and $8\times8$ heterogeneous \ac{MDTC}
...
to 32 addtional transforms for $4\times4$ \acs{TU} and 1 to 128 for $8\times8$
\acp{TU}.
...
As a final observation, the heterogeneous systems learnt iteratively with
...
% TODO
\todo{Add performances of the 128 kB system}
The best performing system, which is labelled as the \SI{128}{\kilo B} has
been tested agains the \ac{HEVC} test set, in order to compare it to the high
performance \ac{MDTC} system from Chapter~\ref{cha:mdtc}.
This system has been learnt using the ToS sequence set, which is completely decorrelated
from the \ac{HEVC} test set.
...
The complexity remains constant for all of them, since the number of
transforms par \ac{IPM} is the same in all systems.
...
\begin{table}[tb]
	...
	\begin{tabular}{c|rr|rr}
		...
		\hline \hline
		 ...
		 32 & \SI{210.00}{\kilo B} & -2.79\% &  \SI{66.00}{\kilo B} &  \\
		 64 & \SI{420.00}{\kilo B} & -3.07\% & \SI{132.00}{\kilo B} &  \\
		128 & \SI{840.00}{\kilo B} & -3.33\% & \SI{264.00}{\kilo B} &  \\
	\end{tabular}
...
% TODO
\todo{Add performances of a high performance symmetrical \acs{MDTC} system}
Table~\ref{tab:sym_mdtc} unveils impact of symmetries for the high performance
\ac{MDTC} system on the 59seq test set.
...
\begin{table}[tb]
	...
	\begin{tabular}{rr|rr}
		...
		\multicolumn{1}{c}{\acs{ROM}} & \multicolumn{1}{c}{Y \acs{BD}-rate} \\
		\hline \hline
		\SI{236.25}{\kilo B} & -3.36\% & \SI{74.25}{\kilo B} & \% \\
	\end{tabular}
...
\subsection{Performances on the \acs{HEVC} test set}
\label{sub:performances_on_the_hevc_test_set}

In order to check the results against a know test set,
table~\ref{tab:bd_rate_mdtc_128kb} contains the bit-rate savings of the best
performing system, labelled as \SI{128}{\kilo B}, compared to the separable
high performance \ac{MDTC} system from Chapter~\ref{cha:mdtc}.

The \SI{128}{\kilo B} \ac{MDTC} system provides lower bit-rate savings for
almost all sequences, although differences are usually small, especially
inside the Class A.
Inside Classes B and C, which were used as the learning set for the separable
high performance \ac{MDTC} system, sequences seem to be especially subject
to over-learning, which around difference of one point in some cases.

Other than that, the independently designed \SI{128}{\kilo B} behaves
correctly, bearing in mind that the learning set used to design the transforms
features only one resolution, frame rate and come from the same short movie.

\begin{table}[tb]
	\centering
	\small
	\begin{tabularx}{0.8\textwidth}{c|X|rr|rr}
		\multicolumn{2}{c}{} &
		\multicolumn{2}{c|}{\SI{128}{\kilo B} \acs{MDTC}} &
		\multicolumn{2}{c}{Sep. High Perf.} \\
		\cline{2-6}
		\multicolumn{1}{c}{} & {Sequence} &
		\multicolumn{1}{c}{ \acs{AI}} & \multicolumn{1}{c|}{ \acs{RA}} &
		\multicolumn{1}{c}{ \acs{AI}} & \multicolumn{1}{c}{ \acs{RA}} \\
		...
		\hline
		\multirow{5}{2cm}{\centering Class A ($2560\times1600$)}
		& NebutaFestival         & -0.99 & -0.08 & -1.06 & -0.08 \\
		& PeopleOnStreet         & -4.24 & -1.58 & -4.21 & -1.49 \\
		& SteamLocomotiveTrain   & -0.51 &  0.29 & -0.60 &  0.23 \\
		& Traffic                & -4.59 & -3.83 & -4.52 & -3.73 \\
		\cline{2-6} &
		Average                  & -2.58 & -1.30 & -2.60 & -1.27 \\
		\hline
		\hline
		\multirow{6}{2cm}{\centering Class B ($1920\times1080$)}
		& BasketballDrive        & -2.88 & -0.69 & -3.22 & -0.72 \\
		& BQTerrace              & -3.84 & -2.27 & -4.70 & -2.65 \\
		& Cactus                 & -4.43 & -2.66 & -5.22 & -3.08 \\
		& Kimono1                & -1.17 & -0.83 & -1.10 & -0.79 \\
		& ParkScene              & -4.80 & -3.39 & -4.61 & -3.22 \\
		\cline{2-6} &
		Average                  & -3.42 & -2.51 & -3.77 & -2.09 \\
		\hline
		\hline
		\multirow{5}{2cm}{\centering Class C ($832\times480$)}
		& BasketballDrill        & -3.53 & -2.39 & -5.92 & -3.53 \\
		& BQMall                 & -4.60 & -2.65 & -4.79 & -2.79 \\
		& PartyScene             & -4.65 & -3.02 & -4.88 & -3.21 \\
		& RaceHorses             & -3.54 & -1.44 & -4.25 & -1.59 \\
		\cline{2-6} &
		Average                  & -4.08 & -2.37 & -4.96 & -2.78 \\
		\hline
		\hline
		\multirow{5}{2cm}{\centering Class D ($416\times240$)}
		& BasketballPass         & -3.62 & -1.62 & -3.91 & -1.82 \\
		& BlowingBubbles         & -3.85 & -2.36 & -4.30 & -2.69 \\
		& BQSquare               & -4.07 & -2.47 & -4.58 & -2.74 \\
		& RaceHorses             & -3.25 & -1.43 & -3.85 & -1.54 \\
		\cline{2-6} &
		Average                  & -3.70 & -1.97 & -4.16 & -2.20 \\
		\hline
		\hline
		\multirow{4}{2cm}{\centering Class E ($1280\times720$)}
		& FourPeople             & -4.18 & -4.77 & -4.56 & -5.18 \\
		& Johnny                 & -3.09 & -3.92 & -3.34 & -4.16 \\
		& KristenAndSara         & -3.37 & -4.19 & -3.82 & -4.47 \\
		\cline{2-6} &
		Average                  & -3.55 & -4.29 & -3.91 & -4.60 \\
		\hline
		\hline
		\multirow{5}{2cm}{\centering Class F (various resolutions)}
		& BasketDrillText        & -4.19 & -2.68 & -6.24 & -3.71 \\
		& ChinaSpeed             & -3.86 & -2.39 & -4.25 & -2.71 \\
		& SlideEditing           & -4.48 & -4.87 & -4.80 & -5.05 \\
		& SlideShow              & -4.98 & -5.00 & -5.66 & -5.61 \\
		\cline{2-6} &
		Average                  & -4.38 & -3.74 & -5.24 & -4.27 \\
		\hline
		\hline
		All sequences &
		Overall                  & -3.61 & -2.51 & -4.10 & -2.76 \\
	\end{tabularx}
	\caption{Y \acs{BD}-rate (\%) for \SI{128}{\kilo B} and separable high
	performance \acs{MDTC} systems}
	\label{tab:bd_rate_mdtc_128kb}
\end{table}


\begin{figure}[tb]
	\centering
	\includegraphics{./figures/mdtc_128kb_plot.pdf}
	\caption{Comparison of \SI{128}{\kilo B} and separable high performace
	\acs{MDTC} systems}
	\label{fig:mdtc_128kb}
\end{figure}


\section{Non-homogeneous symmetrical \acs{MDTC} systems}
\label{sec:non_homogeneous_symmetrical_mdtc}
...
\begin{itemize}
	\item Heterogeneous repartitioning of the number of transforms per \ac{IPM}
...
\begin{figure}[tb]
	...
	\caption{\acs{MDTC} systems making use of heterogeneous transform
...
\begin{figure}[tb]
	...
	\caption{Symmetry impact on non-homogeneous \acs{MDTC} systems}
...
\begin{table}[tb]
	...
	% \SI{128}{\kilo B} & \SI{127.41}{\kilo B} & -3.41\% & 931\% \\
	\end{tabular}
	\caption{Heterogeneous symmetrical \acs{MDTC} systems with \acs{ROM}
	constraints}
	\label{tab:heterogeneous_sym_mdtc}
\end{table}
...
This technique allows saving around two thirds of the \ac{ROM} presented for
...
\ac{ROM} --- Y \ac{BD}-rate trade-offs.

% TODO
\todo{Add more conclusions about systems using both techniques}
...
heterogeneous systems, since there are less transforms to test in each
\ac{IPM}.
...
\ac{ROM} axis, leaving the encoder and decoder complexities decreases to be a
...
\section{The discrete trigonometric transform family}
...
The algorithmic complexity for a \ac{DTT} is in the order of $N\log_2(N)$,
where $N$ stands for the transform
...
\section{\acs{DTT}-based \acs{MDTC} systems}
\label{sec:dtt_based_mdtc_systems}
...
retained.
There are 8 types of \ac{DCT}, 8 types of \ac{DST}, their inverse
versions and, in order to expand the transform space, vertical and horizontal
mirrored versions of the transforms have also been considered.
This adds up to a total of 4096 possible combinations.
Due to relationships between transforms, the actual list contains 2048 unique
transforms.
These transforms are put into a list and iterated through.
Then, the first $N$ transforms are used to classify the residuals.
Afterwards, the transform $N+1$ is replaced in each group and, if it makes the
global \acs{RDOT} metric lower is kept, otherwise is discarded and the same
thing is done for the next transform of the list.

The sub-optimality of this approach is known and the classification algorithm
is subject to be improved, but this simple method will allow to determine the
validity of the approach.
In order to find the optimal \acs{DTT}-based system, one would have to compute
the best combination of $N$ transform from the 2048 available, leading to
\begin{align}
	\left(
	\begin{matrix}
		2048 \\
...
that have to be tested is almost \num{3e14}.
...
% TODO
\todo{add figures for DTT compared to other systems}
...
\begin{table}[tb]
	...
	\end{tabular}
	\caption{Heterogeneous symmetrical \acs{DTT}-based \acs{MDTC} systems with
	\acs{ROM} constraints}
	\label{tab:heterogeneous_sym_dtt_mdtc}
\end{table}
