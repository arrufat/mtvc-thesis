...
indispensable to stream or even store video files:
...
% TODO
\todo{Explain the YCbCr colour space and the target resolutions}
...
The amount of bits to represent the video stream is reduced by limiting the
number of redundancies in the video and by introducing some approximations by
a quantisation step while limiting the impact of these approximations on the
perceived quality.
...
constraints.
There is a deep impact on the coded stream organisation:
\ac{RA}, i.e. the ability to access to a particular piece of a video sequence,
needs to be guaranteed for some applications like TV, while the latency needs
to be kept as low as possible to enable services like surveillance or video
...
standard / \acs{ITU} H.264 recommendation, known as \ac{AVC} or \acs{MPEG}-H
Part 2 / \acs{ITU} H.265, called \acf{HEVC}.
...
The overal coding structure appeared in H.261, in 1988.
...
rectangular blocks of different sizes to be used.
The optimal choice for the block size is left to the encoder.

% TODO
\todo{Explain partitioning figure
\begin{itemize}
	\item intra partitioning
	\item signal divided by uniform regions
	\item make images a bit bigger and put them in the same page
\end{itemize}
}
...
This technique is well know as \ac{DPCM} in the literature.

Those block estimations are carried out by the prediction module, using
some information from previously treated blocks.
This way, predictable information present in the original blocks is
removed and the energy of the resulting signal is lowered so that it requires
less bits for a given distortion.
...
\begin{itemize}
	...
	\item inter prediction, also called temporal prediction for those blocks
		predicted from frames different from the one under consideration.
\end{itemize}

\begin{figure}[tb]
	\centering
	\includegraphics[width=0.8\linewidth]{./figures/partitioning-pred-all-001.png}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.8\linewidth]{./figures/partitioning-resi-all-001.png}
...
to eliminate spatial redundancies by removing the correlation with local
...
Different models of predictions can be used, using projections of adjacent
decoded blocks, theses models include directional projections, gradient
projections (called planar) and the projection of the mean value (called DC).
...
frames and also combine the two origins.
...
It is one of the most demanding parts of video coders, with regard to
...
blocks.
The goal of the transform is to concentrate the residual signal into as
few coefficients as possible in the transform domain.
The residual signal is transformed form the spatial domain into the transform
domain using the transform.
...
approximation operator, which limits the amount of possible output values.
...
For each block size and kind of prediction, the encoder usually computes the
...
% TODO
\todo{Some words on \ac{AI} and \ac{RA}}
...
transformation in order to concentrate the transformed domain residual in as
few coefficients as possible.

The appropriate block size is selected for the prediction and the transform
sizes.
These are the so-called \ac{PU} and \ac{TU} elements in the HEVC syntax.
...
transform.
Typical values vary from $N=4\times4$ to $N=32\times32$ in modern video
codecs, such as \ac{HEVC}.
...
single-dimensional signal. E.g. a $4\times4$ block of pixels becomes a
$16\times1$ vector.
...
% TODO
\todo{illustration on how a $4\times4$ block becomes $16\times1$}
...
$$
\mathbf{A}\otimes\mathbf{B} = \begin{bmatrix} a_{11} \mathbf{B} & \cdots &
a_{1n}\mathbf{B} \\ \vdots & \ddots & \vdots \\ a_{m1} \mathbf{B} & \cdots &
a_{mn} \mathbf{B} \end{bmatrix}
$$
$$
...
those signals in the transform domain.
