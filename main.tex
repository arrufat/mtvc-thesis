...
signals and its efficient implementation has made it the preferred
...
A more detailed view on how it was discovered that the \ac{DST} was a
good approximation of the \ac{KLT} for these signals is detailed in
Chapter~\ref{cha:the_mode_dependent_directional_transforms}, more
precisely in Section~\ref{sec:the_history_behind_the_dst}.
...
which had 9 different prediction modes for intra coding, \ac{HEVC} has a
much improved prediction system, with 35 different prediction modes.
...
\ac{HEVC} $4\times4$ residual by scanning mode, together with their
representation in the transform domain via de \ac{DST}.
The average residual profiles have lower (dark) values near the
available boundaries, which increase with the distance from the
...
It can also be spotted that the scanning patterns match reasonably well
...
\begin{figure}[tb]
	...
	residual signals and their transform using the appropriate scanning}
	\label{fig:mdcs}
\end{figure}

\section{The history behind the \acs{DST}}
\label{sec:the_history_behind_the_dst}
...
that time, nine) was included in the \ac{KTA} software.
The adapted transforms were \acp{KLT}, and the resulting set was called
\ac{MDDT}.
...
Finally, after many \acp{CE} and cross-checks between companies and
