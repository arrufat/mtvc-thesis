...
been tested against the \ac{HEVC} test set, in order to compare it to the high
performance \ac{MDTC} system from Chapter~\ref{cha:mdtc}.
This system has been learnt using the ToS sequence set, which is completely
decorrelated from the \ac{HEVC} test set.
...
Other than that, the independently designed \SI{128}{\kilo B} \ac{MDTC} system
behaves correctly, bearing in mind that the learning set used to design the
transforms features only one resolution, frame rate and coming from the same
short film.

Moreover, the \SI{128}{\kilo B} \ac{MDTC} system needs around half the
\acs{ROM} requirements than the high performance \ac{MDTC} system.
