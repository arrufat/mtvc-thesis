...
The bit-rate savings are over 8\% and it can be seen that the quality of the
sequence improves at low bit-rates (\acs{QP} 37), while effects are less
important as the \acs{QP} increases.
...
direct transform, there are 16 different transforms in total.
Combining them as a horizontal and vertical transforms leads to a total of 256
unique transforms, which are able to capture different kinds of residual
patterns.
...
carried out the other way around if $\A\neq\B$.
...
order of the row and column transforms and then transpose the result.
...
stage, the number of transforms can be increased from 256 to 2048.
Finally, if both transforms are related between each other as $\A^T=\B$,
equations above can all be written in terms of $\A$ and $\A^T$.

Finally, if a different scanning is allowed per transform, both ensembles of
operations are equivalent, since the only difference is the order in which the
transform coefficients are output, but their values remain the same, resulting
in fewer combinations than 256.
These and other relations will be taken into account when designing
...
\begin{figure}[tb]
	...
	\caption{$4\times4$ \acsp{DTT} combination examples}
	\label{fig:some_dtts}
\end{figure}
...
unique transforms, with the exception of a shuffling in the transform domain.
...
\end{align}
Meaning that the number of possible combinations has been reduced by more than
a factor of 4000.
