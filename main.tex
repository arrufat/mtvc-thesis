...
\subsection{Motivation of the \acs{MDDT}}
...
An implementation of the \ac{MDDT} inside a video coding scheme is exemplified
in figure~\ref{fig:mddt_enc}.
...
transform.
The scheme shows that, depending on the \ac{IPM} the residual has been
calculated from, it will present a certain pattern, that will be matched by an
adapted transform.

Figure~\ref{fig:mddt_dec} contains a decoder using the \ac{MDDT} technique.
No additional information has to be sent to the decoder, with regards to the
hybrid video coding scheme:
using the already signalled prediction mode, the decoder will know which
transform to use, without having to perform any calculations or take any
decisions.

There might be an increase in the complexity of both \ac{MDDT} encoder and
decoder, depending on how efficient the implementation of the adapted
transforms is.
...
\subsection{Encoding and decoding schemes for the \acs{MDDT}}
