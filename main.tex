...
of some residuals transformed with the \ac{DST} and the same residuals
...
of values elsewhere.
When changing the transform, the \ac{PDF} is changed, but since the
transforms are orthonormal, only the variance is changes.
\begin{figure}[ht]
	\centering
	\input{./figures/pdfs.pgf}
	\caption{\acsp{PDF} of the transformed coefficients compared to
...
\begin{equation}
	...
	{\e^{-\left(\nicefrac{\vert x \vert}{\alpha}\right)}}^\beta
\end{equation}
...
\begin{align}
	D
	&=\int_{\nicefrac{-\Delta}{2}}^{\nicefrac{\Delta}{2}}
	x^2 \frac{\beta}{2\alpha\Gamma\left(\nicefrac{1}{\beta}\right)}
	{\e^{-\left(\nicefrac{\vert x \vert}{\alpha}\right)}}^\beta \\
	&=2 \int_0^{\nicefrac{\Delta}{2}}
	x^2 \frac{\beta}{2\alpha\Gamma\left(\nicefrac{1}{\beta}\right)}
	{\e^{-\left(\nicefrac{\vert x \vert}{\alpha}\right)}}^\beta \\
	&=\frac{\alpha^2\Gamma\left(
		\frac{3}{\beta},\frac{\Delta^\beta}{\alpha^\beta2^\beta}\right)-
		\alpha^2\Gamma\left(\nicefrac{3}{\beta}\right)}
		{\Gamma \left(\nicefrac{1}{\beta}\right)}
...
\begin{equation}
	...
	\frac{\beta\Delta^2\e^{\nicefrac{-\Delta^\beta}{\alpha^\beta2^\beta}}}
	{8\alpha\Gamma \left(\nicefrac{1}{\beta}\right)}
\end{equation}
...
\begin{align}
	...
	{\e^{-\left(\nicefrac{\vert x \vert}{\alpha}\right)}}^\beta \\
	&=\frac{\Gamma\left(
	\frac{1}{\beta},\frac{{\Delta}^{\beta}}{{\alpha}^{\beta}\,{2}^{\beta}}\right)
	}{\Gamma\left( \nicefrac{1}{\beta}\right) }
	\label{eqn:rate}
\end{align}
...
\begin{equation}
	...
	{8\alpha\Gamma \left(\nicefrac{1}{\beta}\right)}
	-\lambda\frac{\beta\e^{\nicefrac{-\Delta^\beta}{\alpha^\beta2^\beta}}}
	{2\alpha\Gamma \left(\nicefrac{1}{\beta}\right)} = 0
	\quad \Rightarrow \quad \lambda = \frac{\Delta^2}{4}
\end{equation}
...
Lagrange multiplier is independent from the data's \ac{PDF}.
