...
In order to make all results available, the sequence of choice from which
residuals have been output is \emph{Tears of Steel}, a Creative Commons short
film~\cite{blender-tearsofsteel}.
...
From now on, the different sets will be named as:
\begin{itemize}
...
Meaning that the value before the pipe corresponds to the set of sequence that
...
For illustrative purposes, figures~\ref{fig:residuals_independence_4}
...
Steel data set.
For the $8\times8$ case, there seems to be some over-learning in the classes
BC, since it has notable differences in performances with regards the
transforms issued from ToS.
...
Finally, as the 59seq will be serving as the new reference system, against
...
The results are presented in table~\ref{tab:rw_mdtc_59seq}.
Performances of the high complexity \ac{MDTC} system are almost equivalent on
the 59seq testing set, independently of the used learning set.
As a reminder, the separable high performance \ac{MDTC} system had bit-rate
savings of around 4\% on the \ac{HEVC} test set.
\begin{table}[tb]
	\centering
	\small
	\begin{tabular}{c|c}
	Learning set & Y \acs{BD}-rate \\
	\hline\hline
	BC  & -3.5\% \\
	ToS & -3.4\% \\
	\end{tabular}
	\caption{High performance \acs{MDTC} systems on the 59seq using different
	learning sets}
	\label{tab:rw_mdtc_59seq}
\end{table}
% TODO
\todo{Add performances of High Performance MDTC learnt on ToS on BC}
...
equally.
There exists the \acp{MPM}, explained in~\cite{wien-15-hevc}.
The main idea is to favour the \acp{IPM} that are most used during the
encoding of a sequence in terms of signalling.
For this reason, it might be counter productive to add lots of transforms to
those modes that are hardly chosen, making them even less cost efficient,
...
complexity, by limiting the amount of \acs{ROM}, the system is also simplified
in terms of encoding times.
...
Each system contains information about its \acs{ROM} requirements, the
bit-rate savings and the encoding complexity with regards \ac{HEVC}.
...
Algorithm~\ref{alg:vmdtc_iter} present a systematic way of designing a
...
\section{Non-homogeneous symmetrical \acs{MDTC}}
