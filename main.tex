...
increasing the number of transforms.
...
For each \ac{IPM} $2^N$ transforms have been learnt, in addition to the
\ac{HEVC} default transforms.
Algorithm~\ref{alg:clustering} describes how the learning has been carried out
in each \ac{IPM}.
Assuming the desired output are $N$ additional transforms, the performed steps
are:
\begin{enumerate}
	\item Initial random classification of the residuals into $1+N$ classes.
	\item For the $N$ classes that are not assigned to the \ac{HEVC}
...
\end{enumerate}
An example for $N=2$ is provided in figure~\ref{fig:clustering}.

\begin{algorithm}
	\SetKwData{append}{append}
	\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
	\Input{Residuals $\x$ from a given \acl{IPM}}
	\Output{Set of $N$ \acp{RDOT} $A_n$}
	\BlankLine%
...
\begin{figure}[tb]
	...
	\label{fig:rdot_metric_ntransforms}
\end{figure}

\begin{figure}[tb]
	\centering
	\subfloat[\acs{BD}-rate evolution with the number of $4\times4$
	transforms]
	{\includegraphics{./figures/bdrate_ntransforms_4_plot.pdf}}
	\\
	\subfloat[\acs{BD}-rate evolution with the number of $8\times8$
	transforms]
	{\includegraphics{./figures/bdrate_ntransforms_8_plot.pdf}}
	\caption{Average \ac{BD}-rate evolution for the learning set depending
	on the number of transforms}
	\label{fig:bdrate_ntransforms}
\end{figure}
...
\begin{table*}[t]
	...
	\begin{tabular}{c|l|rr|rr|rr|rr}
		...
		& NebutaFestival  &  0.0 &  -0.5 & -0.1 & -0.3 &  -0.1 &  -1.2 & -0.1 & -1.1 \\
		& PeopleOnStreet  & -1.2 &  -2.8 & -0.5 & -1.4 &  -2.3 &  -5.7 & -1.5 & -4.2 \\
		& SteamLocTrain   & -0.4 &  -0.5 &  0.3 & -0.4 &   0.0 &  -0.7 &  0.2 & -0.6 \\
		& Traffic         & -1.7 &  -3.1 & -1.3 & -1.7 &  -5.1 &  -6.1 & -3.7 & -4.5 \\
		\cline{2-10} &
		Average           & -0.9 &  -1.7 & -0.4 & -0.9 &  -1.9 &  -3.4 & -1.3 & -2.6 \\
		\hline
		\multirow{6}{2cm}{\centering Class B ($1920\times1080$)}
		& BasketballDrive & -1.8 &  -3.2 & -0.4 & -1.2 &  -2.4 &  -5.5 & -0.7 & -3.2 \\
		& BQTerrace       & -2.8 &  -5.1 & -1.1 & -1.8 &  -4.9 &  -9.2 & -2.7 & -4.7 \\
		& Cactus          & -2.4 &  -3.8 & -1.2 & -2.0 &  -7.7 & -10.9 & -3.1 & -5.2 \\
		& Kimono1         & -0.5 &  -1.1 & -0.3 & -0.5 &  -1.2 &  -1.8 & -0.8 & -1.1 \\
		& ParkScene       & -1.5 &  -2.2 & -1.1 & -1.7 &  -3.7 &  -5.3 & -3.2 & -4.6 \\
		\cline{2-10} &
		 Average          & -1.8 &  -3.1 & -0.8 & -1.4 &  -4.0 &  -6.6 &  -2.1 & -3.8 \\
		\hline
		\multirow{5}{2cm}{\centering Class C ($832\times480$)}
		& BasketballDrill & -7.1 & -12.8 & -1.3 & -2.1 & -14.8 & -25.1 & -3.5 & -5.9 \\
		& BQMall          & -1.9 &  -3.3 & -1.2 & -2.0 &  -3.6 &  -6.2 & -2.8 & -4.8 \\
		& PartyScene      & -2.3 &  -3.3 & -1.4 & -2.2 &  -4.3 &  -6.2 & -3.2 & -4.9 \\
		& RaceHorses      & -2.2 &  -4.0 & -0.7 & -1.7 &  -3.2 &  -6.8 & -1.6 & -4.3 \\
		\cline{2-10} &
		Average           & -3.4 &  -5.9 & -1.2 & -2.0 &  -6.5 & -11.0 & -2.8 & -5.0 \\
		\hline
		\multirow{5}{2cm}{\centering Class D ($416\times240$)}
		& BasketballPass  & -2.0 &  -3.7 & -0.8 & -1.7 & -3.1  &  -6.2 & -1.8 & -3.9 \\
		& BlowingBubbles  & -2.4 &  -4.2 & -1.2 & -2.0 & -4.0  &  -6.7 & -2.7 & -4.3 \\
		& BQSquare        & -2.0 &  -3.7 & -1.2 & -2.3 & -3.6  &  -6.1 & -2.7 & -4.6 \\
		& RaceHorses      & -2.0 &  -4.6 & -0.6 & -1.6 & -3.2  &  -7.1 & -1.5 & -3.9 \\
		\cline{2-10} &
		Average           & -2.1 &  -4.1 & -1.0 & -1.9 & -4.5  &  -6.5 & -2.2 & -4.2 \\
		\hline
		\multirow{4}{2cm}{\centering Class E ($1280\times720$)}
		& FourPeople      & -3.3 &  -3.4 & -1.9 & -1.8 & -3.1  &  -6.2 & -5.2 & -4.6 \\
		& Johnny          & -2.9 &  -3.0 & -1.6 & -1.3 & -3.o  &  -6.7 & -4.2 & -3.3 \\
		& KristenAndSara  & -3.3 &  -3.5 & -1.8 & -1.3 & -3.6  &  -6.1 & -4.5 & -3.8 \\
		\cline{2-10} &
		Average           & -3.2 &  -4.2 & -1.8 & -2.0 & -6.4  &  -7.7 & -4.4 & -4.7 \\
		\hline
		\multirow{5}{2cm}{\centering Class F (various resolutions)}
		& BasketDrillText & -6.3 & -11.0 & -1.5 & -2.4 & -13.2 & -21.1 & -3.7 & -6.2 \\
		& ChinaSpeed      & -1.6 &  -2.9 & -1.2 & -2.0 &  -3.0 &  -4.9 & -2.7 & -4.3 \\
		& SlideEditing    & -2.0 &  -1.8 & -2.2 & -2.0 &  -4.1 &  -3.7 & -5.1 & -4.8 \\
		& SlideShow       & -3.0 &  -3.5 & -2.3 & -2.5 &  -5.9 &  -6.0 & -5.6 & -5.7 \\
		\cline{2-10} &
		Average           & -3.2 &  -4.8 & -1.8 & -2.2 &  -6.5 &  -9.0 & -4.3 & -5.2 \\
		\hline
		\hline
		All sequences &
		Overall           & -2.4 & -3.8  & -1.1 & -1.7 &  -4.7 &  -7.1 & -2.8 & -4.1 \\
	\end{tabular}
	\caption{Y \acs{BD}-rate (\%) for low complexity and high performance \acs{MDTC} systems}
	\label{tab:bd-rate-mdtc}
\end{table*}
