\begin{tikzpicture}[scale=0.6]
	...
	\draw (90:2.75) node [above] {\textsf{0. Residuals}};
	\pgfmathsetseed{5}
	\foreach \i in {1,2,...,150}{
		\pgfmathsetmacro{\x}{(rand*0.5 + 1)*4 - 4.0}
		\pgfmathsetmacro{\y}{(rand*0.5 + 1)*4 - 4.0}
	...
	\draw (10,0)++(90:2.75) node [above] {\textsf{1. Initial random classification}};

	\draw (5,0) node {\textsf{initialise}};
	...
	\draw (0,\yoffset)++(90:2.75) node [above] {\textsf{2. Learn/update transforms}};
	\pgfmathsetseed{5}
	\foreach \i in {1,2,...,150}{
		\pgfmathsetmacro{\x}{(rand*0.5 + 1)*4 - 4.0}
		\pgfmathsetmacro{\y}{(rand*0.5 + 1)*4 - 4.0 + \yoffset}
	...
	\draw (10,\yoffset)++(90:2.75) node [above] {\textsf{3. Reclassify}};

	\draw[thick,dashed, ->] (3,\yoffset+1) to[in=135,out=45] ++ (4,0);
	\draw (5,\yoffset) node {\textsf{iterate}};
	\draw[thick,dashed, <-] (3,\yoffset-1) to[in=-135,out=-45] ++ (4,0);
\end{tikzpicture}
